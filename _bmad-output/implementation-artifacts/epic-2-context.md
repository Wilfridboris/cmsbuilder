...
- **Roles** are stored authoritatively in `org_members.role` (`admin | member`), per organization: the account creator is always `admin` (recorded at claim time), and invitees get their role recorded at invite time. Server-side role checks read the role from `org_members` (via `requireAdmin` → `resolveUserOrgMembership`, a DB read) on every protected route — never from the JWT / user metadata — so a future role change takes effect immediately and can never be spoofed by a stale token. (A convenience `role` copy is seeded onto an invited user's metadata as initial provisioning, but it is never read for authorization; the claim flow writes no metadata role.)
